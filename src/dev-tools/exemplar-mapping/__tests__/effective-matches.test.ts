...
describe('getEffectiveMatches', () => {
  it('original のみ（added なし）→ そのまま返す', () => {
    ...
    const result = getEffectiveMatches(original, {}, {}, {}, 'fusen-0001', validIds)
  ...
  it('original + added のマージ', () => {
    ...
    const added: Record<string, AddedMatch> = {
      'fusen-0001:ex-003': { isPrimary: false, source: 'manual', reasoning: '手動追加' },
    }
    const result = getEffectiveMatches(original, added, {}, {}, 'fusen-0001', validIds)
    expect(result).toHaveLength(2)
    expect(result[0].exemplarId).toBe('ex-001')
  ...
  it('別の noteId の addedMatches は無視する', () => {
    const original = [makeMatch('ex-001')]
    const added: Record<string, AddedMatch> = {
      'fusen-0002:ex-003': { isPrimary: false, source: 'manual', reasoning: '手動追加' },
    }
    const result = getEffectiveMatches(original, added, {}, {}, 'fusen-0001', validIds)
  ...
  it('dedupe: original 優先で added を捨てる', () => {
    ...
    const added: Record<string, AddedMatch> = {
      'fusen-0001:ex-001': { isPrimary: true, source: 'manual', reasoning: '手動追加' },
    }
    const result = getEffectiveMatches(original, added, {}, {}, 'fusen-0001', validIds)
    expect(result).toHaveLength(1)
  ...
  it('matchStatuses で status を上書き', () => {
    ...
    const statuses = { 'fusen-0001:ex-001': 'approved' as const }
    const result = getEffectiveMatches(original, {}, statuses, {}, 'fusen-0001', validIds)
    expect(result[0].status).toBe('approved')
  })

  it('primaryOverrides で isPrimary を上書き', () => {
    const original = [makeMatch('ex-001', { isPrimary: false })]
    const overrides = { 'fusen-0001:ex-001': true }
    const result = getEffectiveMatches(original, {}, {}, overrides, 'fusen-0001', validIds)
  ...
  it('added の status は pending だが matchStatuses で approved に上書きされる', () => {
    ...
    const statuses = { 'fusen-0001:ex-003': 'approved' as const }
    const result = getEffectiveMatches([], added, statuses, {}, 'fusen-0001', validIds)
    expect(result[0].status).toBe('approved')
  })
  ...
  it('original に Primary がいる状態で added を Primary 追加 → 先頭のみ Primary', () => {
    ...
    const added: Record<string, AddedMatch> = {
      'fusen-0001:ex-003': { isPrimary: true, source: 'manual', reasoning: '手動追加' },
    }
    const result = getEffectiveMatches(original, added, {}, {}, 'fusen-0001', validIds)
    expect(result).toHaveLength(2)
    const primaries = result.filter(m => m.isPrimary)
  ...
  it('added に対する primaryOverrides が効く', () => {
    ...
    const overrides = { 'fusen-0001:ex-003': true }
    const result = getEffectiveMatches([], added, {}, overrides, 'fusen-0001', validIds)
    expect(result[0].isPrimary).toBe(true)
  })

  it('stale な addedMatch に override/status があっても結果に混ざらない', () => {
    const added: Record<string, AddedMatch> = {
      'fusen-0001:ex-STALE': { isPrimary: true, source: 'manual', reasoning: '手動追加' },
    }
    const statuses = { 'fusen-0001:ex-STALE': 'approved' as const }
    const overrides = { 'fusen-0001:ex-STALE': true }
    const result = getEffectiveMatches([], added, statuses, overrides, 'fusen-0001', validIds)
