# pharma-exam-ai プロジェクト AGENTS.md
...
- **付箋→例示マッチング パイプライン（2026-03-26）**
  - 付箋23枚を例示986件にCodex推論でセマンティックマッチング（topicId第一制約）
  ...
  - **次: OCR完了後に1599枚への横展開（1枚ずつCodex推論 × N回、topicId単位でグループ化）→ exemplarIds直接反映 → validateで異常値チェック**
- **付箋→例示マッチングUI改善（2026-03-27）**
  ...
  - テスト: 29ファイル499件全パス
  - **パイプライン判断変更**: マッチング精度が十分（スマホ実機確認済み）なため人間レビューをスキップ。Codex推論（1枚ずつ） → official-notes.ts 直接反映 → validateで異常値チェック。レビューUIは異常値修正用に残す
...
- **暗記フラッシュカード設計（2026-03-31）**
  ...
  - 生成: Codex Opus 4.6（MAXサブスク）、品質検証: Codex Opus + GPT-5.4
...
- **テキストカード生成パイプライン（2026-03-31）**
  ...
  - 生成方式: Codexセッション内サブエージェント（Anthropic SDK API呼び出しではない、MAXプランのトークン使用）
...
- **付箋パイプライン（Phase 2c: 全量パイプライン完走、旧パイプライン完全廃止）**
  ...
  - アノテーションJSON: `/tmp/Codex/fusens/annotation-export-2026-03-26.json`
...
- `npx tsx scripts/split-pages.ts --source makio --input /tmp/Codex/fusens/pages/` — 見開き→左右分割
...
- テキストカードパイプライン: exemplar-stats→フィルタ→過去問集約→Codex Opus atom抽出→バリデーション→科目別JSON
...
- 付箋→例示マッチング: `src/data/fusens/note-exemplar-mappings.json`（1,642件、Codex推論結果、confidence+reasoning付き）
...
- 付箋PDF: Google Drive > pharma-exam-ai > fusen-image > fusen-note-makio.pdf → `/tmp/Codex/fusens/all-subjects.pdf`
