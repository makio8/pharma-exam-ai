...
export function useMappingReviewState() {
  ...
  const togglePrimary = useCallback((noteId: string, exemplarId: string) => {
    setState(prev => {
      const key = `${noteId}:${exemplarId}`
      const current = prev.primaryOverrides[key]
      const next = {
        ...prev,
        primaryOverrides: {
          ...prev.primaryOverrides,
          [key]: current === undefined ? true : !current,
        },
      }
      persist(next)
      return next
  ...
  const resetEntry = useCallback((noteId: string, exemplarIds: string[]) => {
    setState(prev => {
      ...
      for (const eid of exemplarIds) {
        delete newMatchStatuses[`${noteId}:${eid}`]
      }
  ...
  return (
    <div className={candidateClass}>
      ...
      <div className={styles.actions}>
        ...
        <button className={styles.actionBtn} onClick={onTogglePrimary}>
...
interface Props {
  ...
  onTogglePrimary: (noteId: string, exemplarId: string) => void
  onApproveAll: () => void
  onModified: () => void
  onRejectAll: () => void
  onReset: () => void
          ...
          {entry.matches.map(match => {
            ...
            return (
              <ExemplarCandidate
                ...
                onTogglePrimary={() => onTogglePrimary(entry.noteId, match.exemplarId)}
              />
            )
          })}
        </div>
...
const baseContext: ValidationContext = {
  ...
  imageDir: '',
  exemplars: [
    { id: 'ex-physics-001', minorCategory: '化学結合', middleCategoryId: 'physics-material-structure', subject: '物理', text: 'テスト' },
    { id: 'ex-chemistry-001', minorCategory: '基本性質', middleCategoryId: 'chemistry-basic-properties', subject: '化学', text: 'テスト' },
  ],
...
export interface ValidationContext {
  ...
  imageDir: string
  /** 例示マスタ（付箋バリデーション用） */
  exemplars?: Array<{ id: string; middleCategoryId: string; subject: string }>
...
```bash
git add src/utils/data-validator/rules/note-validation.ts src/utils/data-validator/__tests__/note-validation.test.ts src/utils/data-validator/types.ts src/utils/data-validator/index.ts
...
[レビューUI で確認・修正] ← 手動作業
  ↓
Task 10 (official-notes.ts 反映) ← レビュー完了後
