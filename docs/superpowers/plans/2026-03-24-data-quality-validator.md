...
vite.config.ts              ← server.fs.allow に data/pdfs を追加
...
- [ ] **Step 1: vite.config.ts に server.fs.allow 追加**
...
### Task 14: apply-corrections.ts（修正反映スクリプト）
...
2. `baseGitCommit` の祖先チェック（`git merge-base --is-ancestor`）
3. 各問題の `dataHash` 一致確認
4. テキスト/選択肢/正答修正: `exam-{year}.ts` を読み込み → AST変換 or 正規表現置換 → 書き戻し
5. 画像クロップ: PDFから再クロップ（`pdftoppm` + Canvas切り出し）→ PNG上書き
6. 画像削除: `image_url` を `undefined` に
7. 適用ログ出力
8. 自動で `npm run validate` を再実行

- [ ] **Step 2: テスト用の corrections.json を用意してドライラン**
- [ ] **Step 3: コミット + GPT-5.4レビュー**
