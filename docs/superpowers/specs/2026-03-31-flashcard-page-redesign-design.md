...
次回カードタブ訪問時:
  session snapshot が存在 → remainingCardIds を getTemplate(id) で存在検証
    → 全ID存在: 「前回の続き（残りN枚）を再開する？」ダイアログ
    → 一部欠損: 欠損IDを除外して続行提案（「N枚中M枚が更新されました。残りK枚で続行？」）
    → 全ID欠損 or 24時間以上経過: snapshot破棄 → 通常表示
```
